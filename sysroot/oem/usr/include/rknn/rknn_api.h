...
/* asynchronous mode.
   when enable, rknn_outputs_get will not block for too long because it directly
   retrieves the result of the previous frame which can increase the frame rate
   on single-threaded mode, but at the cost of rknn_outputs_get not retrieves
   the result of the current frame. in multi-threaded mode you do not need to
   turn this mode on. */
...
/* collect performance mode.
   ...
   RKNN_QUERY_PERF_DETAIL, ...), but it will reduce the frame rate. */
...
/* dummy init flag: could only get total_weight_size and total_internal_size by
 * rknn_query*/
...
/* set GPU as the preferred execution backend When the operator is not supported
 * by the NPU */
...
/* don't flush input buffer cache, the user must ensure that the input tensor
has flushed the cache before calling rknn_run.
...
/* Don't invalid output buffer cache.
   ...
   you must use rknn_mem_sync (ctx, mem, RKNN_MEMORY_SYNC_FROM_DEVICE) to flush
   the cache. This flags is generally used when the output data of the NPU is
   not accessed by the CPU, but is accessed by the GPU or RGA to reduce the time
   required to flush the cache.
   !!! Don't use this flags when you call rknn_outputs_get() to get output
   data.*/
#define RKNN_FLAG_DISABLE_FLUSH_OUTPUT_MEM_CACHE 0x00008000

/* This flag is used when the model data buffer is allocated by NPU, and can be
 * accessed by NPU directly. */
#define RKNN_FLAG_MODEL_BUFFER_ZERO_COPY 0x00010000

/* This flag is a memory allocation flag, which is used in rknn_create_mem2()
...
#define RKNN_ERR_DEVICE_UNMATCH                        \
  -10 /* the device is unmatch, please update rknn sdk \
...
#define RKNN_MAX_DYNAMIC_SHAPE_NUM \
...
typedef enum _rknn_query_cmd {
  ...
  RKNN_QUERY_NATIVE_INPUT_ATTR =
      8, /* query the attribute of native input tensor. */
  ...
  RKNN_QUERY_NATIVE_NHWC_INPUT_ATTR =
      10, /* query the attribute of native input tensor. */
  RKNN_QUERY_NATIVE_NHWC_OUTPUT_ATTR =
      11, /* query the attribute of native output tensor. */

  RKNN_QUERY_DEVICE_MEM_INFO =
      12, /* query the attribute of rknn memory information. */

  RKNN_QUERY_INPUT_DYNAMIC_RANGE =
      13, /* query the dynamic shape range of rknn input tensor. */
  RKNN_QUERY_CURRENT_INPUT_ATTR =
      14, /* query the current shape of rknn input tensor, only valid
             for  dynamic rknn model*/
  RKNN_QUERY_CURRENT_OUTPUT_ATTR =
      15, /* query the current shape of rknn output tensor, only
             valid for dynamic rknn model*/

  RKNN_QUERY_CURRENT_NATIVE_INPUT_ATTR =
      16, /* query the current native shape of rknn input
             tensor, only valid for  dynamic rknn model*/
  RKNN_QUERY_CURRENT_NATIVE_OUTPUT_ATTR =
      17, /* query the current native shape of rknn output
             tensor, only valid for dynamic rknn model*/
...
typedef enum _rknn_core_mask {
  ...
  RKNN_NPU_CORE_0_1 =
      RKNN_NPU_CORE_0 | RKNN_NPU_CORE_1, /* run on NPU core 0 and core 1. */
  RKNN_NPU_CORE_0_1_2 =
      RKNN_NPU_CORE_0_1 |
      RKNN_NPU_CORE_2, /* run on NPU core 0 and core 1 and core 2. */
  RKNN_NPU_CORE_ALL =
      0xffff, /* auto choice, run on NPU cores depending on platform */
...
typedef struct _rknn_tensor_attr {
  ...
  uint8_t pass_through; /* pass through mode, for rknn_set_io_mem interface.
                           if TRUE, the buf data is passed directly to the input
                           node of the rknn model without any conversion. the
                           following variables do not need to be set. if FALSE,
                           the buf data is converted into an input consistent
                           with the model according to the following type and
                           fmt. so the following variables need to be set.*/
  uint32_t h_stride;    /* the stride along the height dimention of input,
                           Note: it is write-only, if it was set to 0, h_stride =
                           height. */
} rknn_tensor_attr;
...
typedef struct _rknn_input_range {
  ...
  rknn_tensor_format fmt; /* the data format of tensor. */
  char name[RKNN_MAX_NAME_LEN]; /* the name of tensor. */
  uint32_t dyn_range[RKNN_MAX_DYNAMIC_SHAPE_NUM]
                    [RKNN_MAX_DIMS]; /* the dynamic input dimensions range. */
  uint32_t n_dims;                   /* the number of dimensions. */
...
typedef struct _rknn_perf_detail {
  char *perf_data;   /* the string pointer of perf detail. don't need free it by
                        user. */
...
typedef struct _rknn_mem_size {
  ...
  uint32_t total_internal_size;      /* the internal memory size, exclude
                                        inputs/outputs */
...
typedef enum _rknn_tensor_mem_flags {
  RKNN_TENSOR_MEMORY_FLAGS_ALLOC_INSIDE =
      1, /*Used to mark in rknn_destroy_mem() whether it is
         necessary to release the "mem" pointer itself. If the
         flag RKNN_TENSOR_MEMORY_FLAGS_ALLOC_INSIDE is set,
         rknn_destroy_mem() will call free(mem).*/
  RKNN_TENSOR_MEMORY_FLAGS_FROM_FD =
      2, /*Used to mark in rknn_create_mem_from_fd() whether it is
         necessary to release the "mem" pointer itself. If the flag
         RKNN_TENSOR_MEMORY_FLAGS_FROM_FD is set,
         rknn_destroy_mem() will call free(mem).*/
  RKNN_TENSOR_MEMORY_FLAGS_FROM_PHYS =
      3, /*Used to mark in rknn_create_mem_from_phys() whether it
         is necessary to release the "mem" pointer itself. If the
         flag RKNN_TENSOR_MEMORY_FLAGS_FROM_PHYS is set,
         rknn_destroy_mem() will call free(mem).*/
...
typedef enum _rknn_mem_alloc_flags {
  RKNN_FLAG_MEMORY_FLAGS_DEFAULT =
      0 << 0, /* Same with RKNN_FLAG_MEMORY_CACHEABLE */
  ...
  RKNN_FLAG_MEMORY_TRY_ALLOC_SRAM =
      1 << 2, /* Try to allocate memory in SRAM if possible. if SRAM
                 is not enough, allocate rest memory in DRAM. */
...
typedef enum _rknn_mem_sync_mode {
  RKNN_MEMORY_SYNC_TO_DEVICE = 0x1,   /* the mode used for consistency of device
                                         access after CPU accesses data. */
  RKNN_MEMORY_SYNC_FROM_DEVICE = 0x2, /* the mode used for consistency of CPU
                                         access after device accesses data. */
  RKNN_MEMORY_SYNC_BIDIRECTIONAL =
      RKNN_MEMORY_SYNC_TO_DEVICE |
      RKNN_MEMORY_SYNC_FROM_DEVICE, /* the mode used for consistency
                                       of data access between device
                                       and CPU in both directions. */
} rknn_mem_sync_mode;
...
typedef struct _rknn_input {
  ...
  uint8_t
      pass_through;       /* pass through mode.
                             if TRUE, the buf data is passed directly to the input
                             node of the rknn model   without any conversion. the
                             following variables do not need to be set.   if FALSE,
                             the buf data is converted into an input consistent
  ...
  rknn_tensor_format fmt; /* the data format of input buf.
                             ...
                             by default. so entering NCHW data can avoid the
                             format conversion in the driver. */
} rknn_input;
...
typedef struct _rknn_output {
  ...
  uint8_t is_prealloc; /* whether buf is pre-allocated.
                          ...
                          if FALSE, the following variables do not need to be
                          set. */
...
typedef struct _rknn_init_extend {
  ...
  int32_t real_model_offset; /* real rknn model file offset, only valid when
                                init context with rknn file path and zero-copy
                                model model */
  uint32_t real_model_size;  /* real rknn model file size, only valid when init
                                context with rknn file path and zero-copy model
                                model */
...
typedef struct _rknn_output_extend {
  uint64_t frame_id; /* output parameter, indicate the frame id of outputs,
                        corresponds to struct rknn_run_extend.frame_id.*/
} rknn_output_extend;
...
/*  rknn_init
    ...
    input:
        ...
        void* model                 if size > 0, pointer to the rknn model, if
   size = 0, filepath to the rknn model. uint32_t size               the size of
   rknn model. uint32_t flag               extend flag, see the define of
   RKNN_FLAG_XXX_XXX. rknn_init_extend* extend    the extend information of
   init. return: int                         error code.
*/
...
/*  rknn_query
    ...
    return:
        int                         error code.
*/
int rknn_query(rknn_context context, rknn_query_cmd cmd, void *info,
               uint32_t size);
...
/*  rknn_inputs_set
    ...
    input:
        ...
        rknn_input inputs[]         the arrays of inputs information, see
   rknn_input. return: int                         error code
*/
...
/*  rknn_set_core_mask
    ...
    RKNN_NPU_CORE_ALL: auto mode, select multiple npu cores to run depending on
   platform
...
/*  rknn_create_mem_from_fd (memory allocated outside)
    ...
    input:
        ...
        uint32_t size               the size of tensor buffer.
        int32_t offset              indicates the offset of the memory
   (virt_addr without offset). return: rknn_tensor_mem             the pointer
   of tensor memory information.
*/
...
/*  rknn_create_mem_from_mb_blk (memory allocated outside)
    ...
    return:
        rknn_tensor_mem             the pointer of tensor memory information.
*/
rknn_tensor_mem *rknn_create_mem_from_mb_blk(rknn_context ctx, void *mb_blk,
                                             int32_t offset);
...
/*  rknn_create_mem2 (memory allocated inside)
    ...
    return:
        rknn_tensor_mem             the pointer of tensor memory information.
*/
rknn_tensor_mem *rknn_create_mem2(rknn_context ctx, uint64_t size,
...
/*  rknn_set_io_mem
    ...
    input:
        rknn_context ctx            the handle of context.
        rknn_tensor_mem *mem        the array of tensor memory information.
        rknn_tensor_attr *attr      the attribute of input or output tensor
...
/*  rknn_set_input_shapes
    ...
    set all the input tensor shapes. graph will run under current set of input
   shapes after rknn_set_input_shapes.(only valid for dynamic shape rknn model).
    ...
    return:
        int                         error code.
*/
int rknn_set_input_shapes(rknn_context ctx, uint32_t n_inputs,
                          rknn_tensor_attr attr[]);
